...
public class TileManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        StartCoroutine(Shuffle()); //kan bara ropa på metod på detta sätt om det är en Coroutine
    }
    ...
    public bool TilePressed(GameObject tile, bool shuffle)
    {
        ...
        if (emptyNeighbour == null)
            return false;
        float speed = shuffle ? shuffleSpeed : moveSpeed; // lika som if sats ihopdraget
    ...
    private void SwitchTiles(GameObject tile1, GameObject tile2, float speed)
    {
        ...
        LeanTween.move(tile2, pos, speed);
